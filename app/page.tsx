    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <Card key={stat.title} variant="glass" className="group relative overflow-hidden border-2 border-accent-purple">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {stat.title}
                </CardTitle>
                <Icon className={`h-4 w-4 transition-all duration-300 ${
                  stat.title === 'Total Contacts' ? 'text-accent-orange' :
                  stat.title === 'Active Jobs' ? 'text-accent-purple' :
                  stat.title === 'Pending Tasks' ? 'text-accent-orange' :
                  'text-accent-purple'
                }`} />
              </CardHeader>
              <CardContent>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card variant="floating" className="glass-purple">
          <CardHeader>
